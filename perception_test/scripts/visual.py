...
def callback(data):
    ...
    if data.obs_trajs is not None:
        visual_data['obs_trajs'] = np.array(data.obs_trajs).reshape(-1, 16, 2)
        visual_data['pred_trajs'] = np.array(data.pred_trajs).reshape(-1, 3, 24, 2)
            ...
            if obs_trajs is not None:
                for i in range(len(obs_trajs)):
                    ...
                    for j in range(pred_trajs.shape[1]):
                        ...
                        lines = [[j, j+1] for j in range(pred_trajs.shape[2]-1)]
